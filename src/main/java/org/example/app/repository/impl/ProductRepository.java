...
public class ProductRepository implements AppRepository<Product> {
    ...
    public boolean create(Product product) {
        String sql = "INSERT INTO products (productName, quota, price) VALUES (?, ?, ?)";
        return jdbcTemplate.update(sql, product.getProductName(), product.getQuota(), product.getPrice()) > 0;
    }
    ...
    public boolean update(Product product) {
        ...
        if (optional.isEmpty()) return false;
            ...
            String sql = "UPDATE products SET productName = ?, quota = ?, price = ? WHERE id = ?";
            return jdbcTemplate.update(sql, product.getProductName(), product.getQuota(), product.getPrice(), product.getId()) > 0;
        }
    }
